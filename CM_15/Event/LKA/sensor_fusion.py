...
class SensorFusion:
    ...
    Pipeline:
        1. CLAHE contrast enhancement on grayscale
        2. Adaptive white-pixel thresholding (CLAHE + HSV saturation filter)
        3. Perspective warp to bird's-eye view
        4. Sliding-window search to find left & right lane pixel clusters
        5. Polynomial fit through each lane's pixels
        6. Lane center & lateral error computed in BEV pixel space
        7. Convert to meters using calibrated BEV pixel-to-meter ratio

    Key advantages over Hough:
        - Dashed lines are connected naturally by the sliding window
        - Position-based error is correct on curves (BEV removes perspective)
        - No angle/segment-count thresholds to tune
        - Works with fragmented white pixels (no need for continuous edges)
    """
    ...
    def __init__(self):
        ...
        self._frames_dir = os.path.join(_SCRIPT_DIR, "debug_frames", run_tag)
    ...
    def update(self, img, yaw_rate=0.0):
        ...
        try:
            ...
            if self._debug_count % 1 == 0:
                ...
                try:
                    cv2.imwrite(os.path.join(_SCRIPT_DIR, "debug_raw_frame.png"), img)
                    cv2.imwrite(os.path.join(_SCRIPT_DIR, "debug_edges.png"), binary)
                    cv2.imwrite(os.path.join(_SCRIPT_DIR, "debug_bev.png"), bev)
                    ...
                    if sw_vis is not None:
                        ...
                        cv2.imwrite(os.path.join(_SCRIPT_DIR, "debug_sliding_window.png"), sw_vis)
                    ...
                    cv2.imwrite(os.path.join(_SCRIPT_DIR, "debug_vision.png"), vis)
